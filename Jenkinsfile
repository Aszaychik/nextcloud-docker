pipeline {
    ...
    stages {
        ...
        stage('Deploy Stack') {
            steps {
                // Since no secrets are interpolated here, you can stick with double quotes if you prefer
                sh """
        ...
        stage('Verify Deployment') {
            steps {
                sh '''
                    ...
                    timeout 120s bash -c "until docker inspect nextcloud-db --format '{{.State.Health.Status}}' | grep -q healthy; do sleep 5; done"
                    timeout 120s bash -c "until docker inspect nextcloud-app --format '{{.State.Health.Status}}' | grep -q healthy; do sleep 5; done"
